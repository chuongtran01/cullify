...
export function PhotoTile({
  ...
  return (
    ...
    >
      ...
      {state === "pick" ? (
        <Badge className="absolute left-3 top-3 h-auto rounded-full border-transparent bg-primary px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-on-primary">
          AI Pick
        </Badge>
      ) : null}
      {state === "blur" ? (
        <Badge className="absolute left-3 top-3 h-auto rounded-full border-transparent bg-surface-card px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-semantic-error">
